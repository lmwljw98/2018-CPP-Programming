...
void Destroyer::locate(int dMap[][8]){
    ...
    while(true){
        ...
        if(!isVertical){
            if(col <= 0){
                // 오른쪽
                if(dMap[row][col] == 0 && dMap[row][col+1] == 0){
                    for(int i = col; i < col + size; i++){
                        dMap[row][i] = 4; // Defender 맵 배열에 저장
                ...
                // 왼쪽
                else if(dMap[row][col] == 0 && dMap[row][col-1] == 0){
                    for(int i = col; i > col - size; i--){
                        dMap[row][i] = 4; // Defender 맵 배열에 저장
                        row_location[idx] = row;
                        col_location[idx] = i;
        ...
        // 세로 배치
            ...
            // 아래쪽
            if(row <= 0){
                if(dMap[row][col] == 0 && dMap[row+1][col] == 0){
                    for(int i = row; i < row + size; i++){
                        dMap[i][col] = 4; // Defender 맵 배열에 저장
                        row_location[idx] = i;
                ...
                // 위쪽
                else if(dMap[row][col] == 0 && dMap[row-1][col] == 0){
                    for(int i = row; i > row - size; i--){
                        dMap[i][col] = 4; // Defender 맵 배열에 저장
                        row_location[idx] = i;
                        col_location[idx] = col;
